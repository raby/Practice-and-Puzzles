...
int Solution::seats(string A) 
{
	...
	int i = people.size() / 2 - 1;
	int j = people[people.size() / 2] - 1;
	int k = people.size() / 2 + 1;
	int l = people[people.size() / 2] + 1;

	while (i >= 0 || k<people.size())
	{
		if (i >= 0)
			steps = (steps+(j-- - people[i--]))%10000003;
		if (k<people.size())
			steps = (steps+(people[k++]-l++))%10000003;
	}
